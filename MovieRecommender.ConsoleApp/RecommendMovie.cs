    ...
    internal class RecommendMovie
    {
        ...
        public static IEnumerable<Movie> Search(IEnumerable<Movie> movies)
        {
            ...
            if (query != null)
            {
                ...
                {
                    case "T":
                        result = SearchByTitle(query, movies).ToList();
                        break;
                    /*case "A":
                        result = SearchByAuthor(query, movies).ToList();
                        break;*/
                    case "K":
                        result = SearchByKeyword(query, movies).ToList();
                        break;
                    case "G":
                        result = SearchByGenre(query, movies).ToList();
                        break;
                    /*case "D":
                       /* result = SearchByDescription(query, movies).ToList();
                        break;*/
                }
            }
            if (!result.Any())
        ...
        public static void DisplaySearchResults(IEnumerable<Movie> results)
        {
        ...
        private static IEnumerable<Movie> SearchByTitle(string query, IEnumerable<Movie> movies)
        {
            return movies.Where(b => b.Title.Contains(query, StringComparison.InvariantCultureIgnoreCase));
        }

        /*private static IEnumerable<Movie> SearchByAuthor(string query, IEnumerable<Book> books)
        {
            books =
                from b in books
                from a in b.Authors
        ...
        private static IEnumerable<Movie> SearchByKeyword(string query, IEnumerable<Movie> movies)
        {
            return movies.Where(b => b.Keywords.Any(x => x.Contains(query, StringComparison.InvariantCultureIgnoreCase)));
        }

        private static IEnumerable<Movie> SearchByGenre(string query, IEnumerable<Movie> movies)
        {
            return movies.Where(b => b.Genre.ToString().Contains(query, StringComparison.InvariantCultureIgnoreCase));
        }
